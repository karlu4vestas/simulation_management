...
When the cleanup cycle starts then the cleanup_status_date. The owner and approvers are notified when the cleanup cycle starts and 2 workdays before it ends.
...
- **`next`** (cleanup_frequency days): Mandatory state. 
   ...
   - Changing the retention of other simulations to this state sets `retention_expiration_date = cleanup_status_date + cleanup_frequency`
...
Example: when changing the retention of a simulation from `marked` days to `90d` using the UI sets `retention_expiration_date = cleanup_status_date + 90 days`
...
- If a simulation was produced 1 day into the cleanup round then its retention expire in 1 day (`modified_date-cleanup_status_date`) and will be assigned to the `next` retention label
...
The following shows:
- An entire cleanup cycle (defines cleanup_status_date) and the start of the next cleanup cycle
...
4. **Cleanup Execution (Day 30)**:
   ...
   - Next cycle begins with new `cleanup_status_date`
