...
A secure folder always starts in a state with `frequency="inactive"`, meaning that the cleanup process is inactive.
...
The cleanup frequency can be changed or paused (`frequency="inactive"`):
...
- **`next`** (frequency days): Mandatory state. 
   ...
   - Changing the retention of other simulations to this state sets `retention_expiration_date = cleanup_round_start_date + frequency`
...
- **`next`**:    `0 < days_to_cleanup ≤ frequency`
- **`90d`**:     `2 × frequency < days_to_cleanup ≤ 90 days`
...
i = 0
cat = filter(retention_catalog, lambda e: e.value is not null)   // [0, frequency, 90, 180, 365, 730, 1095]
cat_days_to_cleanup = cat.values   // [0, frequency, 90, 180, 365, 730, 1095]
...
- if a simulation' retention is changed to `next` then then its retention expire in `frequency` days and will be assigned the `next` retention label
