...
class IO:
    ...
    def write(file_path, data_to_write) :
        ...
        while failures < 5 and succes is False:
            try:
                with open(file_path, 'wb', buffering=IO.size_of_buffer) as file:
    ...
    def read(file_path) :
        ...
        while failures < 5 and succes is False:
            try:
                with open(file_path, 'rb', buffering=IO.size_of_buffer) as f:
                    content = f.read() 
                succes = True
            except IOError:
                failures = failures + 1
                ...
                with os.scandir(root_folder) as ite:
                    for entry in ite:
                        ...
                        if entry.is_file(follow_symlinks=False):
                            files.append( p )
                        elif entry.is_dirdir(follow_symlinks=False):
                            dirs.append( p )
                succes = True
            except IOError:
                failures = failures + 1
