...
from GenerateTimeseries import CleanStatus, Base_Simulation_Generator, GenerateTimeseries, SimulationLoadcaseType, SimulationType, TimeseriesNames_vs_LoadcaseNames, LoadcaseConfiguration, generate_random_files
from loadcases.loadcases_lc_synth_from_ranges import write_synthetic_setfile
from GenerateTimeseries_all_pr_ext import Generate_clean_all_pr_ext
from GenerateTimeseries_all_but_one_pr_ext import Generate_clean_all_but_one_pr_ext
...
class GenerateSimulation (Base_Simulation_Generator) :
    ...
    def __init__(self, base_path:str, loadcase_ranges_filepath:str, 
        ...
        self.timeseries_generators:dict[str, GenerateTimeseries] = { 
                                       "int": Generate_clean_all_pr_ext(self, 0,         ["INT"],            [".int", ".tff"] ), 
                                       #"sim": Generate_clean_all_pr_ext(self, 0,         ["EXTFND","DAT"],   [".sim"]         ), 
                                       "sim": Generate_clean_all_pr_ext(self, 0,         ["EXTFND"],         [".sim"]         ), 
                                       "out": Generate_clean_all_but_one_pr_ext(self, 0, ["OUT"],            [".out"]         ), 
                                       "sta": Generate_clean_all_but_one_pr_ext(self, 0, ["STA"],            [".sta"]         ), 
                                       "eig": Generate_clean_all_but_one_pr_ext(self, 0, ["EIG"],            [".eig", ".mtx"] ), 
                                       "log": Generate_clean_all_but_one_pr_ext(self, 0, ["LOG"],            [".log"]         ) 
                                    }
    ...
    def generate_INPUTS(self) -> tuple[dict[str,str], dict[str,list[str]]]:        
        input_path:str            = os.path.join(self.base_path,"INPUTS")
        input_files:dict[str,str] = {path:CleanStatus.KEEP for path in generate_random_files(10, input_path, "set")}
    ...
    def generatefiles(self):
        ...
        all_file_status, setfiles_names = self.generate_INPUTS()
        ...
        timeserie_generators: list[GenerateTimeseries] = list(self.timeseries_generators.values())
        with ThreadPoolExecutor() as executor:
            for d in executor.map(lambda gen: gen.generate(self.base_path, self.timeseries_names_vs_loadcase), timeserie_generators) :
                all_file_status.update(d)        

        # return the expected clean status for all generated files 
        return all_file_status