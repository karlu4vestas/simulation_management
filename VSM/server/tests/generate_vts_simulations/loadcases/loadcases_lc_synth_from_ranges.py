...
def synth_header_lines(rnd, on_off=None):
    ...
    blank = ""  # Blank line after site (required by VTS format)
    hdr = "LC file n_rot Vhub Gen Wdir Turb Vexp rho Wind Turbfil Title"
    units = "[rpm] [m/s] [deg]"
    return [prep, title, site, blank, hdr, units]
...
def write_synthetic_setfile(out_path, num, seed, sim_json):
    ...
    with open(out_path, 'w', encoding='utf-8') as w:
        ...
        for i in range(num):
            ...
            if params:
                w.write(main + " >>\n")
                # Write all continuation parameters on one line with indentation
                w.write(" " * 72 + " ".join(params) + "\n")
            else:
                w.write(main + "\n")
